...
function storeTaskInLocalStorage(task) {
  ...
  if(localStorage.getItem('tasks') === null) {
    ...
    tasks = JSON.parse(localStorage.getItem('task'));  //local storage can only store strings so we 'change'-PARSE as string
  }

  tasks.push(task);

  localStorage.setItem('task', JSON.stringify(task));  //must be stored as string hence json
}
